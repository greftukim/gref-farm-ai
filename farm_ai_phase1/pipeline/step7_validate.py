...
def run(full_chain_csv: str = None,
    ...
    report = (
        "=== GREF Phase 1.5 Validation Report ===\n"
        "(S&V 시차: 착과기 수확 7주 전 / SV_EC_LAG=1)\n\n"
        "[단계별 오차율]\n"
        f"TOMGRO 단독:       {mape_tomgro:.1f}%  (기대: 25.6%)\n"
        f"+ S&V:             {mape_sv:.1f}%  (Phase 1: 35.5%)\n"
        f"+ XGBoost 5-fold:  {mape_final:.1f}%  (기대: 21.5%)\n\n"
        "[월별 오차율 (XGBoost CV)]\n"
