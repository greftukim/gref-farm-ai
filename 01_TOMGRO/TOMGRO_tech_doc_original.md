...
TOMGRO (Tomato Growth) 는 1991년 미국 Florida 대학교의 Jones 교수팀이 개발한 **토마토 생육-수확 시뮬레이션 모델**입니다. 이후 네덜란드 Wageningen 대학의 Heuvelink (1996) 가 박사 학위 논문으로 개선 버전을 발표했고, 이것이 현재 전 세계 온실 농업 연구의 표준이 되었습니다.
...
TOMGRO는 온실 환경(일사, CO₂, 온도)과 작물 상태(LAI)로부터 작물의 **광합성 속도와 건물 생산량**을 예측합니다. 본 프로젝트에서는 이 중 핵심 모듈인 **광합성 계산부**를 구현하여 1작기 실측 데이터에 적용했습니다.
...
TOMGRO는 "이 환경 조건에서 작물이 이론적으로 만들 수 있는 건물량의 **상한선**"을 제공합니다. 이후 S&V, 적과, XGBoost가 각자의 손실·보정을 적용하며 실제 수확량에 수렴해갑니다.

### 0.3 한계 (먼저 명시)

TOMGRO 단독으로는 실제 수확량을 정확히 예측할 수 없습니다.
...
본 섹션에서 **"MAPE 95.7%"** 라는 수치가 나오더라도 이는 "TOMGRO가 틀렸다"가 아니라 **"TOMGRO는 건물 생산 단계까지만 계산한다"** 는 의미로 이해하셔야 합니다.
...
매주 1회 측정 기록. TOMGRO의 LAI 입력 변수 계산에 사용됩니다.
...
**근거**: TOMGRO는 "1시간 동안 지속된 환경에 대한 광합성량"을 계산합니다. 5분 간격 12개 측정값의 평균이 그 시간의 대표 환경을 잘 나타냅니다.

**예시** (2025-11-10 12:00~13:00 동안 12개 5분 데이터):
```
radiation 측정값 (5분 간격):
688.5, 691.2, 695.8, 702.1, 698.3, 693.7, 696.9, 694.2, 690.5, 695.1, 696.8, 703.5 (W/m²)
→ 평균 = 695.23 W/m²   (이것이 "11/10 12시대" 대표 radiation)
```

### 1.4 전처리 2: 일사량 → PAR 변환

TOMGRO는 **PAR (Photosynthetically Active Radiation, 광합성 유효 광량)** 을 입력으로 받습니다. 이는 380~700nm 파장의 광량만 나타내며, 전체 태양 복사 에너지의 약 45%에 해당합니다.
...
LAI는 원본 PRIVA 데이터에는 없으므로 **주간 생육 측정값**에서 계산합니다.
...
**근거**: 환경 변수는 시간적 연속성이 있고, 1% 수준의 단발성 결측은 전후 값의 평균으로 복원 가능합니다. 대규모 연속 결측은 해당 기간을 분석에서 제외했습니다.

---

## 섹션 2 — TOMGRO 광합성 수식

**이 섹션의 모든 수식은 원문 논문 그대로입니다. 본 프로젝트에서 수정하지 않았습니다.**
...
**구현**: 캐노피를 **5개 층**으로 균등 분할하여 각 층의 빛을 계산합니다.
...
5개 층은 수치적 안정성과 계산 부담의 균형점입니다. Heuvelink (1996) 는 3~7개 층을 권장하며, 본 프로젝트는 중간값 선택.

### 2.3 Step 2: 잎 단위 광합성

**직사각 쌍곡선** (Rectangular Hyperbola) 형태:
...
$$f_{CO_2} = \frac{CO_2 - \Gamma}{CO_2 - \Gamma + K_{CO_2}} \div \frac{350 - \Gamma}{350 - \Gamma + K_{CO_2}}$$
...
- 1/10⁶: μmol → mol
- 30 g/mol: CH₂O (포도당 단위) 몰질량

최종 단위: **g CH₂O / m²(ground) / hour**

### 2.7 유지호흡 (Maintenance Respiration)

광합성으로 만든 탄수화물 중 일부는 기존 조직을 유지하는 데 쓰입니다 (유지호흡). 온도가 높을수록 호흡이 빨라집니다.
...
**변환 요소**:
- **30/44**: CH₂O (30 g/mol) → DM 몰질량 관계 환산 계수
...
TOMGRO는 매일 생산된 순 건물을 세 기관으로 배분합니다:
- **잎** (leaf): 광합성 생산
- **줄기** (stem): 구조 지지
- **과실** (fruit): 수확 대상

배분 비율은 **DAP (Days After Planting, 정식 후 경과일)** 에 따라 변합니다.
...
이 주의 DAP는 120~126일이므로 **생식생장 안정기** 입니다.
...
| **방울토마토** | **0.07 (7%)** | Adams (1990), Wu et al. (2006) |
...
이 예시는 **TOMGRO의 내부 작동을 완전히 추적** 하기 위한 사례입니다. 원본 데이터부터 최종 출력까지 모든 숫자를 따라갈 수 있습니다.
...
11/12 하루 시간별로 4단계 처리 결과를 시각화한 것입니다:
...
11/12는 흐린 날이라 PAR 피크가 546으로 낮습니다 (비교: 11/10 정오 1404 μmol/m²/s).
...
TOMGRO는 "이론적 건물 상한선" 만 계산하므로 다음이 빠져 있습니다:
...
S&V + 적과를 적용하면 예측이 **0.661 kg/m²** (S&V 14% 손실) → **0.652 kg/m²** (적과 1.4% 추가 손실) 로 감소하며 MAPE도 내려갑니다. XGBoost가 잔차를 학습하면 추가 개선이 기대되지만 1작기 데이터로는 과적합 한계 있음.
...
**비선형 포화 관계** 확인됨. 직사각쌍곡선 수식이 실제 패턴과 잘 일치.
...
- Wu, M. et al. (2006) "Water and dry matter distribution in cherry tomato under different water supply conditions" — 방울토마토 7.0~7.5% 측정
...
이런 물리 파라미터를 조정하면 **과적합**이 되어 다른 Scene이나 다른 작기에서 일반화가 안 됩니다.
...
이 수치는 TOMGRO가 나쁘다는 뜻이 아니라, **TOMGRO가 건물 생산 단계까지만 계산한다**는 사실을 반영합니다.
